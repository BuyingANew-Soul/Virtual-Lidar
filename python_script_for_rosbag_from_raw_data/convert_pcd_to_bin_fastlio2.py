...
def convert_pcd_to_bin(pcd_dir, output_bin):
    """Convert PCD files to BIN format including intensity and ring fields, excluding time."""
    ...
    with open(output_bin, "wb") as f_bin:
        for pcd_file in pcd_files:
            ...
            # ✅ Try extracting intensity if available
            ...
            for i in range(len(points)):
                ring = i % 16  # ✅ Assign ring ID for VLP-16 (0-15)

                # Write x, y, z, intensity, ring (NO time)
                f_bin.write(struct.pack("ffffH", points[i][0], points[i][1], points[i][2], intensity[i], ring))

    print(f"✅ Converted {len(pcd_files)} PCD files to {output_bin} (including intensity & ring, but NO time)")

convert_pcd_to_bin(
    "/home/zero/VirtualLidar/univ_outdoor_summer_bright_sunny_2019-09-09-14-41-41_raw/vlp/pcd",
